...
double romberg(double f(double), double a, double b, int IT, double Precisao, double **R);
 ...
 FILE *arqERRO; 
  ...
  R[0][0] = Trapz(f, a, b, t , 0);
    ...
    arqERRO= fopen("ERRO.txt", "w+");
      ...
      {
         printf("\n");
         ERRO[i] = 2.746801534 - R[i][i]; // Aqui o usuário deve digitar o valor exato da integral.
...
int main()
{
  ...
  romberg(Func, a, b,15,Precisao, R);
